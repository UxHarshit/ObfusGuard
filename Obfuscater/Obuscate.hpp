...
#define _XCF RND(1, 10)
#define _XCA RND(1, 7)

#define _T_R_U_E_ ((((__4() * _n_2_) + (_n_6_ / __3()) + (_XCF % 2)) - (_n_5_ - __2()))/ _n_3_)
#define _F_A_L_S_E_ (((((__5() * _n_3_) + (_n_4_ / __1()) + (_XCF % 3)) - (_n_7_ - __6())) / _n_8_) * __0())

//Control flow functions
#define FUNC_PROCESS(c, b) if(c) { b; }
#define FUNC_R_PROCESS(c, b) if(c && _XCF) { b; }
#define FUNC_TRUE(b) FUNC_PROCESS(_T_R_U_E_, b)
#define FUNC_FALSE(b) FUNC_PROCESS(_F_A_L_S_E_, b)
#define FUNC_R_TRUE(b) FUNC_R_PROCESS(_T_R_U_E_, b)
...
volatile static __attribute__((noinline)) int proxy_wrap(double val) {
    volatile double a = val * ((double) (_XCA) - ((double) _n_3_ * 2));
    FUNC_TRUE(
            FUNC_FALSE(
                    a = val * ((double) (_XCA) - ((double) _n_3_ * 2));
    )
            FUNC_TRUE(
            a = val * ((double)(_XCA) - ((double)_n_3_ * 2));
    )
    ...
    #define if(cond) \
                    if (_T_R_U_E_) \
                        if (proxy_wrap(cond) * _T_R_U_E_ && _XCF)
    #define else else \
                    FUNC_FALSE( \
                        proxy_wrap(_XCF); \
                        FUNC_R_TRUE( \
